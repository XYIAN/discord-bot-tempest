      ...
      embeds: [
        stormEmbed(
          ...
          [
            ...
            '',
            `🌫️ You start as a **${ctx.guild.roles.member.name}** — chat to climb the storm ranks, from ${ctx.guild.activityTiers[0]?.role.name ?? 'Storm Recruit'} all the way to ${ctx.guild.activityTiers.at(-1)?.role.name ?? 'Eye of the Storm'}.`,
            ...
            `⚔️ Want to step inside the storm? The **${identity.guildName}** guild recruits daily-active players at ${identity.requiredPower}+ power — see ${channelMention(ctx, member, ctx.guild.channels.recruiting)}.`,
          ].join('\n'),
      ...
      embeds: [
        stormEmbed(
          ...
          [
            ...
            '• `/rank` tracks your activity as you chat — everyone starts as a Storm Watcher and climbs from there',
